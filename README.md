...
| 屏幕取词 | 首页开关 → 常驻通知栏 + 可拖动悬浮球 → 在任意应用里点一下即可截屏识别、点选翻译；图片与文字都不离开设备 |
...
OCR 部分另有：多边形几何（旋转卡壳最小外接矩形、unclip 外扩、四点透视裁剪、角点排序）、
CTC 贪心解码、段落合并、字典字符表校验、语种覆盖一致性、图像旋转与坐标逆变换，
以及**用桌面版 ONNX Runtime 跑真实 PP-OCRv6 模型**的端到端识别测试（对内置样例截图断言识别出的
文字，含「截图被转了 90°」「竖排对话与横排说明混排」两种朝向场景）。
...
覆盖：模型装载与 SHA-256 校验、设备端真实识别（内置样例截图与竖排漫画图，断言文字内容、朝向与
耗时上限）、屏幕取词界面（空态、选中计数、按钮可用性）、常驻入口的启动与撤销、
悬浮球的几何（贴边吸附、拖动与轻点判定）、历史与导航。

通知相关测试需要先授权：`adb shell pm grant com.lingua.app android.permission.POST_NOTIFICATIONS`
（未授权时它们会自动跳过）。
...
**三种进入方式**：首页的「屏幕翻译」开关（下方详述）、从系统分享面板把一张截图发给灵译
（`ACTION_SEND` / `image/*`）、或在应用内用系统照片选择器挑一张图片。
...
首页（翻译页）的「屏幕翻译」开关打开后，会同时得到两个入口：通知栏里的一条常驻通知，
以及一个可拖动的悬浮球。之后切到任意应用，**点悬浮球或点通知**即可翻译当前屏幕。

- **开关每次打开应用都会回到关闭**：它不写进 DataStore，是「现在要用」的控件而不是设置项。
  冷启动主界面时还会主动停掉服务，连带撤掉通知与悬浮球，避免忘记关掉的入口一直挂着。
- 悬浮球可以拖动，松手后吸附到较近的一侧屏幕边缘；轻点才是触发，拖动不会误触发。
- 常驻入口由 `ScreenTranslateService`（前台服务）承载：悬浮球是归属进程的 overlay，没有前台服务
  的话，用户一离开应用它就消失了 —— 而那正是需要它的时候。真正截屏那一刻再启动
  `mediaProjection` 类型的前台服务取一帧，取完立刻停止。
- 权限：Android 13+ 需要通知权限，「显示在其他应用上层」需要用户到系统页面授权。打开开关会按
  顺序申请这两项，都拿到后才真正开启；缺少哪一项，开关旁的说明就直接写哪一项。通知上的「关闭」
  按钮可一键撤掉通知与悬浮球，不必回到应用；回到应用时开关会与入口的实际状态对齐。
- Android 15+ 还有一层系统级的「屏幕共享防护」：在部分 ROM 上（实测 ColorOS / Android 16）它会让
  第三方应用的截屏一直返回黑帧。需要在开发者选项里「停用屏幕共享防护」，详见下方已知限制。

**为什么点一下就能截到别的应用**：

1. 触发的 `ScreenTranslateActivity` 用全透明主题，并在清单里声明了 `taskAffinity=""` ——
   后者是关键，否则启动它会顺带把灵译自己的主界面带到前台，截到的就是灵译而不是你正在看的应用。
   该 Activity 在手握一帧之前什么都不画，所以合成到屏幕上的只有下方应用。
2. 悬浮球在触发时先被摘掉：overlay 窗口同样会被屏幕捕获，不摘掉就会把自己截进自己的截图里
   （截图流程结束、Activity 销毁时再放回来）。
3. 这也是应用内不再提供「截取屏幕」按钮的原因：从灵译自己的界面里截屏，能截到的只有灵译自己。

已知的系统行为：**平台的「设置」应用声明了 `HIDE_OVERLAY_WINDOWS`，聚焦时会隐藏第三方悬浮窗**
（防点击劫持），所以在系统设置里悬浮球会暂时不可见，通知入口仍然可用。
...
**朝向处理**：识别模型只读横排。正立那一遍若是干净结果（平均置信度 ≥ 0.9）就直接返回，不做多余
推理；分数偏低时才把整图分别旋转 90° / 180° / 270° 再试，取平均置信度最高的朝向，并把检测框逆变换
回原图坐标，界面上会标出「已旋转 N° 后识别」。同一屏混排（漫画的竖排对话 + 横排说明）时，两个朝向
的结果会按框的重叠度去重、保留置信度更高的那份，因此两种情况都能读出来。实测竖排日文四列全部
正确（各列置信度 1.00），横排说明文字也能同时保留。
...
| 情况 | 表现 |
| --- | --- |
| 竖排日文的阅读顺序 | 按从左到右输出，而日文竖排习惯从右到左；每一列的文字本身是正确的 |
| 旋转角度不是 90° 的整数倍 | 倾斜文本的检测框正确但识别会失败 |
| `FLAG_SECURE` 界面（银行、部分应用） | 截出来是纯色，会提示「该界面禁止截屏」 |
| 通知权限被拒（Android 13+） | 常驻通知无法显示，首页开关旁的说明会提示去系统设置授权 |
| 未授予「显示在其他应用上层」 | 悬浮球不出现，通知入口照常可用，开关旁的说明会提示去授权 |
| 系统设置等声明了 `HIDE_OVERLAY_WINDOWS` 的应用 | 聚焦时悬浮球被系统隐藏（防点击劫持），离开后自动恢复 |
| 每次截屏的系统确认 | Android 14+ 的 `MediaProjection` 授权是按次的，无法「授权一次长期可用」；已改用「整个屏幕」配置，只需一次点击（不再需要选择分享哪个应用） |
| Android 15+ 的屏幕共享防护 | 系统自 Android 15 起对屏幕共享做隐私保护，部分 ROM（实测 ColorOS / Android 16）会让第三方应用的整屏捕获一直返回纯黑帧，界面因此提示「无法截取该界面」。豁免方式：**开发者选项 → 停用屏幕共享防护**（对应 `Settings.Global.DISABLE_SCREEN_SHARE_PROTECTIONS_FOR_APPS_AND_NOTIFICATIONS=1`）；系统自带录屏不受该防护影响 |

...
| 部分定制 ROM 的截屏策略 | 实测 ColorOS（Android 16）对第三方应用的 `MediaProjection` 多数时候返回纯黑帧，表现为提示「无法截取该界面」；同一份代码在 AOSP / Waydroid 上正常。`PROJECT_MEDIA` app-op 为 allow、屏幕常亮、系统日志无拒绝记录，疑似厂商隐私保护策略 |

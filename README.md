...
| 屏幕取词 | 首页开关 → 常驻通知栏 → 在任意应用里点按通知即可截屏识别、点选翻译；图片与文字都不离开设备 |
...
耗时上限）、屏幕取词界面（空态、选中计数、按钮可用性）、常驻通知的发布与撤销、历史与导航。
...
首页（翻译页）的「屏幕翻译」开关打开后，应用会在通知栏常驻一条通知；之后切到任意应用，
**点按通知**即可翻译当前屏幕。

- **开关每次打开应用都会回到关闭**：它不写进 DataStore，是「现在要用」的控件而不是设置项。
  冷启动主界面时还会主动撤掉遗留通知，避免忘记关掉的通知一直挂着。
- 通知本身不是前台服务：它只需要活得比进程久，而通知天然如此。真正截屏那一刻才启动
  `mediaProjection` 类型的前台服务，取到一帧立刻停止。
- Android 13+ 需要通知权限：没有权限时开关旁的说明会直接提示，打开开关会先申请权限，
  授权后自动生效，不用再点一次。
- 通知上的「关闭」按钮可直接撤掉通知，不必回到应用；回到应用时开关会与通知的实际状态对齐。

**为什么点通知能截到别的应用**：通知拉起的 `ScreenTranslateActivity` 用全透明主题，并且在清单里
声明了 `taskAffinity=""` —— 后者是关键，否则启动它会顺带把灵译自己的主界面带到前台，截到的就是
灵译而不是你正在看的应用。该 Activity 在手握一帧之前什么都不画，所以合成到屏幕上的只有下方应用。
这也是应用内不再提供「截取屏幕」按钮的原因：从灵译自己的界面里截屏，能截到的只有灵译自己。
